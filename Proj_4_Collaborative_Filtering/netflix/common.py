...
def plot_mine(X,X2, mixture,mixture2, post,post2,title,title2,overtit):
    """Plots the mixture model for 2D data"""

    fig, (ax1,ax2) = plt.subplots(1,2,figsize=(10,15))
    fig.suptitle(overtit)
    _, K = post.shape
    percent = post / post.sum(axis=1).reshape(-1, 1)
    ...
    for j in range(K):
        ...
        legend = "mu = ({:0.2f}, {:0.2f})\n stdv = {:0.2f}".format(
            mu[0], mu[1], sigma)
        ax1.text(mu[0], mu[1], legend)
    #plt.axis('equal')

    _, K = post2.shape
    percent = post2 / post2.sum(axis=1).reshape(-1, 1)
    ...
    for j in range(K):
        mu = mixture2.mu[j]
        sigma = np.sqrt(mixture2.var[j])
        circle = Circle(mu, sigma, color=color[j], fill=False)
        ax2.add_patch(circle)
        legend = "mu = ({:0.2f}, {:0.2f})\n stdv = {:0.2f}".format(
            mu[0], mu[1], sigma)
        ax2.text(mu[0], mu[1], legend)
    #plt.axis('equal')
    plt.show()
...
def bic(X: np.ndarray, mixture: GaussianMixture,
    ...
    BIC = log_likelihood - (0.5) * (K-1+K+(K*d)) * np.log(n)
    return BIC
